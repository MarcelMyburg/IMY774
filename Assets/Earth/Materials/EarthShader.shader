	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				o.normal = WorldSpaceVertexPos(float4(v.normal, 1.0)).xyz;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 worldNormal = normalize(vertexNormal + (normalMapNormal.xxy * _NormalMapIntensity * float3(-1.0, 1.0, -1.0)));

				fixed specular = tex2D(_SpecularTex, i.uv).a;
				float smooth = _Smoothness;
				fixed rim = 1.0 - saturate(dot(normalize(rayDir), vertexNormal));
				fixed3 emission = (_RimColor.rgb * pow(rim, _RimPower) + AtmosphereColor(-rayDir));

				fixed3 h = normalize(_SunDir + rayDir);
				fixed d = max(0, dot(vertexNormal, _SunDir));
				fixed e = max(0, dot(worldNormal, _SunDir));
				fixed nh = max(0, dot(vertexNormal, h));
